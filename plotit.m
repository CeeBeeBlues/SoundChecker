...
% fix on 11-4-2016 to include ice files
ccc
...
fprintf(1,'Do you need to run or rerun the excel?(y/n)\n');
fprintf(1,'(say y if any results have changed for the selected stage,\n');
rerun = input('or if results have not be run yet)','s');
...
if iceorno == 'y';
        [folderpathice] = uigetdir('\\nmfs.local\AKC-NMML\CAEP\Acoustics\Physical Data (Oceano, Meteo, etc)\ice\', ...
            'Pick an ice folder');
end
...
dc1 = nums(thisMooring-1,1);% recOnTime
dc2 = nums(thisMooring-1,2);% Duty Cycle
...
for m = 1:size(udays,1); % for each day (m)
    ...
    if m > 1;
        ...
        if maxtimeperpng > timediff; % has the potential to span into the next day
            % but check whether the next day starts with 00:00
            timing =  datestr(vectx(pngs(1)),13);
            dvtiming = datevec(timing);
            if sum(dvtiming(4:6)) == 0
                % do nothing
                extra = 0;
            else
...
for S = 1:size(Spp,1);
    ...
    fnan = find(isnan(percentCalling)== 1);
    fnanyandm = find(isnan(percentCallingyandm)== 1);
    
    percentCalling(fnan) = 999;
       percentCallingyandM(fnanyandm) = 999;
...
for spp = 1:size(Spp,1);
    ...
    if iceorno == 'y';
        if spp == 1; % only do this for the first spp, all the rest are same locn.
            allUnderscores = strfind(PngFile,'_');
            Prj = PngFile(allUnderscores(1)+1:allUnderscores(1)+2);
            if strmatch(Prj,'BS'); % this is an Bering Sea M mooring.
                Mooring = ['M' PngFile(allUnderscores(3)+1:allUnderscores(3)+2)];
            elseif strmatch(Prj,'BF'); % this is an old Bowfest mooring
                Mooring = ['BF' PngFile(allUnderscores(3)+2:allUnderscores(3)+2)];
            elseif strmatch(Prj,'RW');
                Ye = str2num( PngFile(allUnderscores(1)+3:allUnderscores(1)+4));
                if Ye >= 10;
                    Mooring = ['BS' PngFile(allUnderscores(3)+2:allUnderscores(3)+2)];
                elseif Ye == 8;
                    moornum =  PngFile(allUnderscores(3)+2:allUnderscores(3)+2);
                    if moornum == 1;
                        Mooring = ['BS7'];
                    elseif moornum == 2;
                        Mooring = ['BS8'];
                ...
                Mooring = PngFile(allUnderscores(3)+1:allUnderscores(4)-1);
            end
            findIce = strmatch(Mooring,Ices(:,1:3));
    ...
    years = [str2num(datestr(datestart,'YYYY')):str2num(datestr(dateend,'YYYY'))+1];
    mos = 1:12;
    days = [1 15];
    stagl = 0.01;
    tickdates = [];
    ...
    % overlay the grayed areas for no data
    nans = isnan(x);
    p = find(nans);
    
   
    if sum(nans)==0; % there are no NaN's
        % do nothing'
    else
        bb1 = bar(dates,100*nans,...
...
printfilename =  [ExcelFile(1:end-4) extratextstuff icepref];

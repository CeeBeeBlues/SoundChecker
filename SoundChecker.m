...
Sounds(2).Spp = {'right', 'bowhead','gunshot','humpback','minke',...
    'gray','walrus','genPinni','vessel','airguns','wtf', 'dblKnck'};
...
if size(checkfiles,1) > 0;
    ...
    % a bit of an edit for the quizes
    if Quiz == 'y';
        
            findwhichone = strncmp([lower(WhoRan) '.mat'], cellstr(lower(rsltFiles(:,end-6:end))),3);
            findnumone = find(findwhichone == 1);
            if sum(findwhichone) > 0;
                findCheck = findnumone;
            else
                findCheck = [];
            end
...
if isempty(findCheck); % if that check doesn't exist.
    ...
    if Quiz == 'y';
        PngRsltFileName = [PngRsltFileName(1:end-4) '_' lower(WhoRan) '.mat'];
    end
    cd(PngFolder);
    PngMonths = ls;
...
elseif ~isempty(findCheck); % if that check exists.
...
elseif strcmp(PngType,'SHI') == 1; % super high  % dont' need to adjust for
    ...
    fs = PNGrslts_MetaData(CheckNum).maxHz*2;
    PNGrslts_MetaData(CheckNum).secondsPerRow = .75*60;
...
if Quiz == 'n';
    if days2go >= 1;
        tic
        dailyLOG(pngs2go,AnalysisFolder, WhoRan, Recorder,CheckNum);
        toc
    end
...
if Quiz == 'y';
    butt.objBlocker = uicontrol('Parent',f1,'Style','Text', ...
    'String', 'UNIDENTIFIED','backgroundcolor',[1 1 1], ...
    'Units','normalized','Position',[.25 .92 .45 .25]);
end
...
for o = 1:size(handleMTX,1);  % for each species make a button group
    ...
    a3 = uicontrol('Parent', eval(['butt.objgrp' num2str(o)]),'visible','on',...
        ...
        'if exist(''excelFlag'',''var''); var1 = excelApp; var2 = excelWorkbook; else; var1 = 1; var2 = 2; end; [excelApp, excelWorkbook,excelFlag] = notes(notesFile,fileMTX,n,m,Sounds,CheckNum,WhoRan,butt,var1,var2,f2);');
...
butt.obj2 = uicontrol('Parent', f2,'Style', 'pushbutton', ...
    ...
    'PNGrslts_MetaData,handleMTX,WhoRan,pngCounter,f1,f2);']);
...
butt.hold = uicontrol('Parent', f2,'Style', 'togglebutton', ...
    ...
    ' holdMode(butt,resltMTX,handleMTX,n,m,f2);']);
...
butt.zoom = uicontrol('Parent', f1,'Style', 'pushbutton', ...
    ...
    'PNGrslts_MetaData, CheckNum,n,m,WhoRan,f2,Quiz,currentContrast);']);


butt.numrows = uicontrol('Parent', f1,'Style', 'edit', ...
    'Units','normalized','Position',[.92 .75 .02 .02],...
    'String', num2str(PNGrslts_MetaData(CheckNum).rowsPerPlot), 'FontSize',8,...
    'Enable','on','Callback', ...
    ['[PNGrslts_MetaData,fs] = getnumrows(PNGrslts_MetaData, butt,CheckNum) ']);
...
butt.dark = uicontrol('Parent', f2,'Style','pushbutton', ...
    ...
    'fileMTX,n,m,f1,f2,PNGrslts_MetaData,pngCounter,resltMTX,whoMTX,CheckNum,' ...

function [excelApp, excelWorkbook,excelFlag] = notes(notesFile,fileMTX,n,m,Sounds,CheckNum,WhoRan,butt,var1,var2,f2);
% CLB
%v4.0 04/04/13 - multi-line works now
%v3.0 12/28/12 - multiple species review now

if get(butt.review,'Value') == 1;
    % you need different n and m's here.
    Getvars = get(f2,'WindowKeyPressFcn'); 
    reviewCntr = Getvars{21}; % this is the review counter number.
    RevList = get(butt.review,'UserData');
    n = RevList(reviewCntr+2,2);
    m = RevList(reviewCntr+2,1);
...
% and write to it
if get(butt.review,'Value') == 0;
    fprintf(fid,'%s,%s,%s,%s,%s\n',png, char(Species),noteTextnocomma,WhoRan,datestr(now));
else
    fprintf(fid,'%s,%s,%s,%s,%s\n',png, char(Species),['REV' noteTextnocomma],WhoRan,datestr(now));
end
% and close it
fclose(fid);

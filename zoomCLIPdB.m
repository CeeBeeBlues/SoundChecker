function [zoomclip,wavtimeMIN,wavtimeMAX,fs,yHzmn,yHzmx] = zoomCLIPdB(butt,PngFolder,WaveFolder,...
    fileMTX,PNGrslts_MetaData,CheckNum,n,m,WhoRan,f2,Quiz,currentContrast)
%
...
PngFileDashes = strfind(fileMTX{m}(n,:),'-');  %look at that particular file
pngfile = fileMTX{m}(n,:);
...
if pngfile(1:2) == 'SB';
    ...
    % have to find that recorder first.
    fwf = strmatch(pngfile(1:PngFileDashes(2)-1),wavs);
    thisWav = max(find(wavsDateTime(fwf,:) <= pngDateTime))
    thisWav = fwf(thisWav)
end
...
zoomTitle = [fileMTX{m}(n,1:PngFileDashes(2)) pngDateTimezoom '-' ...
    num2str(currentContrast) fileMTX{m}(n, PngFileDashes(4)+3:end)];
title(zoomTitle);
...
S.SpeciesAbbrevs = PNGrslts_MetaData(CheckNum).CheckSpp;
 ...
 S.Quiz = Quiz;
 S.mo = mo; % month folder in Pngs and Waves
 S.PngFolder = PngFolder;
 S.currentContrast = currentContrast;
...
FileTitle = [S.fileName(1,1:S.PngFileDashes(2)) S.pngDateTimezoom ...
    '-' num2str(S.currentContrast)];%     '-' S.fileName(1, S.PngFileDashes(4)+1:S.PngFileDashes(4)+2)];
...
% now save the full png in the same folder and rename a bit
 %not going to give the option to do this because everyone says they'll say
 %yes, so it saves an extra click.

% get day here
dayName = S.fileName(S.PngFileDashes(2)+1:S.PngFileDashes(3)-1);
% get correct contrast here
newfileName = [S.fileName];
newfileName(S.PngFileDashes(4)+1:S.PngFileDashes(4)+2) = num2str(S.currentContrast);
% also need correct folder for that contrast
findSlashes = findstr('\', S.PngFolder);
endslash = findSlashes(end);
setCD = cd;
    cd(S.PngFolder(1:endslash));
     ldBs = dir;
    ldBs = char(ldBs.name);
    ldbsCell = cellstr(ldBs)';
    findNewCont = strfind(ldbsCell,num2str(S.currentContrast));
     whichContFold = find(~cellfun(@isempty,findNewCont));
     cd(setCD) % switch folder back
    
MainPngFolder = deblank([S.PngFolder(1:endslash) ldBs(whichContFold,:)])

copyfile([MainPngFolder '\' deblank(S.mo) '\' dayName '\' newfileName],[zoomclipFolder '\' FileTitle '_full.png']);
...
% There seems to be a problem playing non-standardized sampling rate clips
% through gmail.  It is a product of the rounding of the sampling rate.  

%The following code resamples the clip up to the standard 44.1kHz CD
%quality file that all players should be able to handle. switch back if
%this gives more problems.
 rsClip = resample(Z.clip/normlz,44100,round(Z.fs));
wavwrite(rsClip,44100,Z.nbits,[zoomclipFolder '\' FileTitle '.wav'])
%The following line was working for all but the gmail direct play
